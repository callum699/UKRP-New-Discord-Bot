...
@bot.tree.command(name="scamlink", description="Delete user messages and timeout for 24 hours across all guilds")
@app_commands.describe(
    user="User who posted the scam link",
    delete_range="How many hours back to delete messages"
)
async def scamlink(interaction: discord.Interaction, user: discord.User, delete_range: int = 24):
    ...
    # ✅ ONLY this (NO other response before it)
    ...
    await interaction.followup.send(
        f"🛑 Scam action complete\n"
        f"🔇 Timed out in: {success_timeout} servers\n"
        f"🗑️ Messages deleted: {success_messages}"
    )
